...
<body class="bg-gray-100 min-h-screen p-4">  <div class="max-w-5xl mx-auto grid md:grid-cols-3 gap-5"><div class="md:col-span-2 bg-white rounded-3xl shadow-xl p-5">

  <div class="flex justify-between items-center mb-6">
    <div>
      <h1 class="text-3xl font-bold">WhatsApp Work Manager</h1>
      <p class="text-gray-500 mt-1">Professional Work Tracking System</p>
    </div>

    <div class="relative">
      <button onclick="toggleMenu()"
        class="bg-gray-200 px-4 py-2 rounded-xl text-2xl">
        ⋮
      </button>
  ...
  <div class="space-y-4">
    ...
    <div class="relative">
      ...
      <div id="numberSelector"
        ...
        <label class="flex items-center gap-3 p-4 border-b cursor-pointer hover:bg-gray-50">
    ...
    <textarea id="work"
      ...
      class="w-full border p-4 rounded-2xl h-32 outline-none"></textarea>

    

    <button onclick="saveWork()"
      class="w-full bg-green-600 hover:bg-green-700 text-white p-4 rounded-2xl text-lg font-bold">
      Save & Send WhatsApp
    </button>
  </div>
</div>
...
<div class="bg-white rounded-3xl shadow-xl p-5">
  <div class="flex justify-between items-center mb-5">
    <div>
      ...
      <p class="text-gray-500 text-sm">Permanent Saved Records</p>
    </div>

    <button onclick="resetFilter()"
      class="bg-gray-200 px-3 py-2 rounded-xl text-sm">
      Reset
    </button>
  </div>
  ...
  <div class="mt-5 bg-blue-100 text-blue-700 p-4 rounded-2xl text-sm font-semibold cursor-pointer" onclick="showAllHistory()">
    📂 Permanent All History
  </div>

  <div class="mt-3 bg-red-100 text-red-700 p-4 rounded-2xl text-sm font-semibold cursor-pointer" onclick="deleteAllHistory()">
    🗑️ Permanent Delete All History
  </div>

  <div class="mt-3 bg-red-100 text-red-600 p-4 rounded-2xl text-sm">
...
function renderNumbers() {
  ...
  if (numbers.length === 0) {
    numberList.innerHTML = '<div class="p-4 text-gray-500">No History</div>';
    return;
  }
  ...
  numbers.forEach(number => {
    ...
    btn.innerHTML = `
      ...
      <div class="text-sm text-gray-500">View History</div>
    `;
...
function renderHistory() {
  ...
  if (filtered.length === 0) {
    historyBox.innerHTML = '<div class="text-center text-gray-500 py-10">No History Found</div>';
    return;
  }
  ...
  filtered.forEach(item => {
    ...
    card.className = 'bg-gray-50 border rounded-3xl p-5';
    ...
    card.innerHTML = `
      ...
      <div class="mb-3 inline-block bg-green-100 text-green-700 px-4 py-2 rounded-full text-sm font-bold">
        ₹ ${item.amount}
      </div>

      <div class="bg-white p-4 rounded-2xl border text-sm leading-6">
        ${item.work}
      </div>
